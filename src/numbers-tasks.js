...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  // throw new Error('Not implemented');
  const eq1 = x1 * x2 + y1 * y2;
  const eq2 = Math.sqrt(x1 ** 2 + y1 ** 2);
  const eq3 = Math.sqrt(x2 ** 2 + y2 ** 2);

  return Math.acos(eq1 / (eq2 * eq3));
}
...
function getSumOfDigits(num) {
  ...
  // const sum = numArray.map(Number).reduce(function reducer(a, b) {
  //   return a + b;
  // }, 0);
